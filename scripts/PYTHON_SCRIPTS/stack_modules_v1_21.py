...
def attach_user_to_group(**args):
	...
	try:
		...
		login_response = iam.create_login_profile(UserName = args['UserName'], Password = "2Tobechanged!", PasswordResetRequired = True)
...
def Migrate_On_Prem_Users(**kwargs):
	...
	try:
		...
		for names in usernames:
			...
			aws_create_user(service="iam", user=extracted_name)
		
			# Calling the AWS add user to group function
			attach_user_to_group(service="iam", UserName=extracted_name, GroupName=sys.argv[2])
		...
		if error.add_user_to_group_response['Error']['Code'] == "NoSuchEntity":
			print(error.attach_user_response)
			print("User doesnt not exist")
