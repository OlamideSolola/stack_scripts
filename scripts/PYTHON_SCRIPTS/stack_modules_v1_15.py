...
def db_connection(**args):
	...
	connection=c.connect(user="STACK_TEMP", password="stackinc", dsn="MKIT-DEV-OEM/APEXDB")
	...
	if args["args15"] == status_before_job_run:
		...
		# Creating the tables and Columns to be created
	...
	# Checking if the job status is completed
		...
		cursor.execute("""update operations set OP_ENDTIME = :OP_ENDTIME_INS, STATUS = :STATUS_INS where RUNNER = :RUNNER_INS and OP_ID = :OP_ID_INS""",
		OP_ENDTIME_INS=args['args17'],
		STATUS_INS=args['args15'],
		RUNNER_INS=args['args14'],
		OP_ID_INS=args['args10'])
	...
	# Checking if the job status is "error"
		...
		# Updating the created row with endtime information
		cursor.execute("""update operations set OP_ENDTIME = :OP_ENDTIME_INS, STATUS = :STATUS_INS where RUNNER = :RUNNER_INS""",
		OP_ENDTIME_INS=args['args17'],
		STATUS_INS=args['args15'],
		RUNNER_INS=args['args14'])
...
def database_backup(**args):
	...
	db_connection(args15=STATUS_INS, args10=OP_ID_INS, args11=OP_NAME_INS, args12=OP_STARTTIME_INS, args14= RUNNER_INS, args16=OP_TYPE_INS, args17=OP_ENDTIME_INS)
...
def f_d_copy(**args):
	...
	status_before_job_run, status_after_job_run, status_job_error = status
	STATUS_INS = status_before_job_run
	OP_ID_INS = sys.argv[4]
	OP_NAME_INS = sys.argv[1]
	OP_STARTTIME_INS = current_datetime_formated
	RUNNER_INS = sys.argv[5]
	OP_TYPE_INS = sys.argv[6]
	OP_ENDTIME_INS = current_datetime_formated
	...
	try:
		...
		while 2 > 1:
			...
			if use_percent > int(args["args3"]):
				...
				BODY="The disk utilization for {} is above the warning Threshold!".format(disk_path)
				FROM="oracle@MKIT-DEV-OEM.localdomain"
				...
				BODY="The disk utilization for {} is above warning Threshold!!!".format(disk_path)
				FROM="oracle@MKIT-DEV-OEM.localdomain"
...
def data_migration(**args):
	...
	try:
		...
		database_backup(args1=schema, args2=runner, args3=directory)
