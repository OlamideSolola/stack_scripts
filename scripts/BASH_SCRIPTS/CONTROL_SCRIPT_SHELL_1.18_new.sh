...
EOF
		...
		echo "dumpfile=${schema}_${runner}_${TS}.dmp" >> expdp.par
		echo "logfile=${schema}_${runner}_${TS}.log" >> expdp.par
		...
		file_name=${schema}_${runner}_${TS}.tar
		files=${schema}_${runner}

		#cd into the backup directory
		cd /backup/AWSSEP23/APEXDB
		
		#Zipping up the dump file
		tar -cvf ${file_name} ${files}*.dmp --remove-files
		...
		echo "An APEXDB instance is not running"
		exit
	fi
...
then
	...
	read -p "How many schemas do you want to back up? " NO_SCHEMAS
	
	if [[ $NO_SCHEMAS == 1 ]]
	then
		read -p "Specify the schema to backup: " SCHEMA1
	...
	then
		read -p "Specify the first schema: " SCHEMA1
		read -p "Specify the second schema: " SCHEMA2
		read -p "Specify directory: "  DIRECTOTY
		read -p "Specify the runner: " RUNNER
		...
		echo "You can only backup 2 schemas for now"
...
then
	...
	schema1=`echo $SCHEMAS | awk '{print $1}'`
	schema2=`echo $SCHEMAS | awk '{print $2}'`
	directory=$3
	runner=$4		
	TS=`date '+%m%d%y%H%M%S'`
