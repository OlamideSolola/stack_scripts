...
backup_f_d ()
{
	...
	#Checking the number of command line argument and scping if we have 7 command line args
	if (( $# == 7 )) 
	then
	...
	then
		...
		echo "Backed up files successfully copied to the cloud"
	fi
...
EOF
		...
		tar -cvf /backup/AWSSEP23/APEXDB/${each_schema}_${runner}_${TS}.tar /backup/AWSSEP23/APEXDB/*${each_schema}_${runner} --remove-files
...
then
	...
	then 
		echo "The backup function with an scp to the cloud has been selected"
		source=$2
		destination=$3
		runner=$4
		priv_key=$5
	...
	##Calling the backup function
	backup_f_d $source $destination $runner $priv_key $dest_server $dest_dir
	...
	echo "If you want to SCP the backup to the cloud, pls enter a private key, destination server and destination directory"
	echo "Eg ./CONTROL_SCRIPT_SHELL_1.10.sh source destination runner priv_key dest_server dest_dir"
	exit
