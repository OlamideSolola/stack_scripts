...
database_backup()
{
	...
	#checking to see if we have the an APEXDB instance running
...
EOF
		...
		if ( ls -ltr /backup/AWSSEP23/APEXDB/stack_temp_olamide_solola.log )
			then
				echo " Backup sucessfull. Log file exists"
		fi	
	else
